...
List:元素是有序存储的，可以重复。
Set:元素是无序存储的，不可以重复。
Map:使用键值对存储，key不可以重复，value可以重复。
```
#### List
```
ArrayList: Object[]数组
...
链表和红黑树主要解决hash冲突，由key的hashCode经过hash算法得到hash值，通过hash&(n-1)得到key在数组的下标， 如果当前位置存在元素，则比较key判断出是新增到链表尾部还是更新key的value，如果当前位置不存在元素，则把该元素放到当前位置即可。
加载因子loadFactor： 控制数组的疏密程度，太大hash冲突就会增加，降低查找效率；太小内存利用率低。
阈值threashold：等于capacity*loadFactor，衡量数组是否需要扩容的标准。
put过程：
1.判断table是否已经初始化，如果没有初始化则调扩容的方法，新建数组并把容量设置为大于传入的参数的2的n次方。
2.计算key的hashCode的hash值得到key在数组的位置，如果位置没有元素直接插入；
如果当前节点等于要插入的元素则更新value；
如果节点为红黑树，则红黑树插入；
